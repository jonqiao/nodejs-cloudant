...
The best way to use the Cloudant client is to begin with your own Node.js
project, and define this work as your dependency. In other words, put me in your
package.json dependencies. The `npm` tool can do this for you, from the command
line:

    $ npm install --save @cloudant/cloudant

Notice that your package.json will now reflect this package. Everything is
working if you can run this command with no errors:
...
and supplying a callback function to run when everything is ready.
...
Upper-case `Cloudant` is this package you load using `require()`, while
lower-case `cloudant` represents an authenticated, confirmed connection to your
Cloudant service.

If you omit the "password" field, you will get an "anonymous" connection: a
client that sends no authentication information (no passwords, no cookies, etc.)
...
async function asyncCall() {
  ...
  return await cloudant.use('alice').insert({ happy: true }, 'rabbit');
}
...
To use Cloudant, add `require('@cloudant/cloudant')` in your code. In general,
the common style is that `Cloudant` (upper-case) is the **package** you load;
whereas `cloudant` (lower-case) is your connection to your database (i.e. the
result of calling `Cloudant()`).
...
You can just pass your account name and password (see the [security
note](#security-note) about placing your password into your source code).
...
~~~ js
var cloudant = Cloudant({ url:"companycloudant.local", username:"somebody", password:"somebody's secret" });
~~~
...
function. This will make the library automatically "ping" Cloudant to confirm
the connection and that your credentials work.
...
Cloudant API keys are part of the legacy access controls. They are diferent from
...
If you need to access your Cloudant database from a web application that is
served from a domain other than your Cloudant account, you will need to enable
...
- To enable access from a list of specified domains:
  ~~~ js
  cloudant.set_cors({ enable_cors: true, allow_credentials: true, origins: [ "https://mydomain.com","https://mysubdomain.mydomain.com"]}).then((data) => {
    // success - response is in 'data'.
  }).catch((err) => {
    // failure - error information is in 'err'.
  });
